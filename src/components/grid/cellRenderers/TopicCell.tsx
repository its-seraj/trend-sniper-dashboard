...
export function TopicCell(props: CustomCellRendererProps) {
  const candidates = props.value as TopicCandidate[]
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      >
        {candidates.map((c) => (
          <span key={c.topic} data-badge style={badgeStyle}>
            {c.topic}
          </span>
        ))}
      </div>
      ...
      <div ref={containerRef} style={{ display: 'flex', gap: '4px', alignItems: 'center', overflow: 'hidden', width: '100%', height: '100%' }}>
        {candidates.slice(0, visible).map((c) => (
          <span key={c.topic} data-badge title={`score: ${c.score} · ${c.matches.join(', ')}`} style={badgeStyle}>
            {c.topic}
          </span>
        ))}
        {hidden > 0 && (
          <span title={candidates.slice(visible).map(c => c.topic).join(', ')} style={overflowStyle}>
            +{hidden}
          </span>
        )}
      </div>
